...
float old_humidity;
float tempThreshold = 0.1; // порог сравнения в предыдущими показаниями температуры
float humThreshold = 0.1; // порог сравнения в предыдущими показаниями влажности  test
int16_t nRFRSSI;
int16_t old_nRFRSSI;
int16_t myid;
int16_t mypar;
...
void readData() {
  ...
  if (temperature < -40) {
    temperatureSend = -40.0;
  }
  if (temperature > 125) {
    temperatureSend = 125.0;
  }
  ...
  if (flag_nogateway_mode == false) {
    ...
    if (hch == true) {
      ...
      if (check == false) {
        _transportSM.failedUplinkTransmissions = 0;
        wait(50);
        check = send(msgHum.set(humiditySend, 2));  // change_data
        if (check == false) {
          wait(150);
          _transportSM.failedUplinkTransmissions = 0;
          check = send(msgHum.set(humiditySend, 2));
